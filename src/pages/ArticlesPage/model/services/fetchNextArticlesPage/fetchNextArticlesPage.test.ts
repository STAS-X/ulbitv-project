...
describe('fetchNextArticlesPage test', () => {
	...
	test('should fulfilled without data', async () => {
		...
		expect(testThunk.api.get).not.toHaveBeenCalled();
	...
	test('should fulfilled', async () => {
		testThunk.api.get.mockReturnValue(Promise.resolve({ data: [articleData], headers: { 'x-total-count': '1' } }));
		const result = await testThunk.callThunk();
	...
	test('should rejected', async () => {
		...
		expect(result.payload).toBe('error occured');
	});
});
