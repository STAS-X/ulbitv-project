describe('Тесты на динамический компонент подгрузки статей для страницы ArticlesPage', () => {
	...
	it('Тестируем подгрузку статей на странице', () => {
		...
		cy.getByTestId('ArticleSortSelector')
			.should('exist', true)
			.getByTestId('Button.TILE')
			.click()
			.invoke('attr', 'class')
			.should('match', / _selected_/);
	...
	it('Тестируем фильтрацию статей по тематике', () => {
		...
		cy.getByTestId('ArticleCategorySelector')
			.should('exist', true)
			.getByTestId('Article.Category.SCIENCE')
			.as('Category.SCIENCE')
			.click()
			.invoke('attr', 'class')
			.should('match', / _selected_/);
	...
	it('Тестируем сортировку статей по Просмотрам на убывание/возрастание', () => {
		...
		cy.getByTestId('ArticleSortSelector')
			.should('exist', true)
			.getByTestId('Article.Sort.Field.Value')
			.select('Просмотры')
			.should('have.value', 'views');
		cy.getByTestId('ArticleSortSelector')
			.should('exist', true)
			.getByTestId('Article.Sort.Order.Value')
			.select('возрастание')
	...
	it('Тестируем фильтрацию статей по полю поиска', () => {
		...
		cy.getByTestId('ArtricleSearch')
			.should('exist', true)
			.getByTestId('Article.Filter.Value')
			.as('Filter')
			.clear()
			.type('Visual')
			.should('have.value', 'Visual');
